...
async def store_request_history(user_id: str, endpoint: str, request_data: Dict[str, Any]) -> str:
    """Store request history in Redis."""
    r = await get_redis()
    request_id = hashlib.md5(f"{user_id}:{endpoint}:{json.dumps(request_data)}".encode()).hexdigest()
